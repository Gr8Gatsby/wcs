...
function assertCustomEmojis(customEmojis) {
    ...
    const firstItemIsFaulty =
        ...
        customEmojis.length &&
        (!customEmojis[0] ||
            requiredKeys.some((key) => !(key in customEmojis[0])));
...
function customEmojiIndex(customEmojis) {
    ...
    const emojiToTokens = (emoji) => [
        ...new Set(
            emoji.shortcodes.map((shortcode) => extractTokens(shortcode)).flat()
        )
    ];
...
function assertEmojiBaseData(emojiBaseData) {
    if (
        ...
        throw new Error(
            'Expected emojibase full (not compact) data, but data is in wrong format'
        );
    }
}
